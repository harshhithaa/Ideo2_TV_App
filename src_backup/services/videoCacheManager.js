...
class VideoCacheManager {
    ...
    try {
      ...
      if (data) {
        try {
          ...
          console.log('[Cache] Metadata corrupted, resetting cache');
          this.metadata = { files: {}, totalSize: 0 };
      ...
      while (attempt < maxRetries) {
        try {
          ...
          if (result.statusCode === 200) {
            ...
            // ✅ ADD: Store file size for later memory management
            this.metadata.files[mediaRef] = {
              path: cachePath,
              size: actualSize,
              lastAccessed: Date.now(),
              downloadDate: Date.now(),
              isLargeFile: actualSize > 500 * 1024 * 1024, // ✅ Flag for special handling
            };
            this.metadata.totalSize += actualSize;
            await this.saveMetadata();

            console.log(`[Cache] ✓ Complete: ${(actualSize / 1024 / 1024).toFixed(1)}MB${this.metadata.files[mediaRef].isLargeFile ? ' [LARGE]' : ''}`);
  ...
  // ✅ ADD: New method to check if file is large
  isLargeFile(mediaRef) {
    return this.metadata?.files?.[mediaRef]?.isLargeFile || false;
  }

  // ✅ ADD: Get file size from metadata
  getFileMetadata(mediaRef) {
    return this.metadata?.files?.[mediaRef] || null;
}
}
